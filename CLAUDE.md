...
## A probe that reports every route as clean is on one route

Written up separately because it is the most useful thing this pass produced.

The teaching screens measured **21/21 clean** at six viewports -- no unnamed
controls, no overflow, no console errors -- and every single configuration
reported `h1="Welcome back"`. **The same heading on three different routes is
the tell.** It had never left the landing page: preview mode is read once when
its provider mounts, so writing the sessionStorage key and then navigating
gives the provider nothing to read, and `RequireAuth` sent all 21 to
`#/sign-in`.
...
the strings. `teaching.test.ts` now holds every `to=` on those screens against
